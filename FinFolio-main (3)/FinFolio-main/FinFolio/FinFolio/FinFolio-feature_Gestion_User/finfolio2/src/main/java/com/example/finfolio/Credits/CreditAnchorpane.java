...
import static com.example.finfolio.Credits.PDFGenerator.generatePDF;
...
public class CreditAnchorpane {
    ...
    private void displayRatings(List<User> userList) throws SQLException {
        for (User user : userList) {
            OffreService offreService = new OffreService();
            double userRating = offreService.calculateAndDisplayRatingForUsers(user.getId());
            // Create a label to display rating
            // Add userRatingLabel to your UI layout (e.g., VBox, GridPane, etc.)
            // Example: vbox.getChildren().add(userRatingLabel);
        }
    }
    ...
    private void handleSendButton() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/finfolio/User/sendOffre.fxml"));
            ...
            sendOffreController.setUserID(1);
    ...
    public void generatepdfclick(ActionEvent actionEvent) throws FileNotFoundException {
    ...
    void contacterbutton(ActionEvent event) {
        try {
            ...
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/finfolio/User/chatroom.fxml"));
            Parent root = loader.load();
