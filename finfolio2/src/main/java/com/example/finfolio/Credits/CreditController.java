...
////////////////////////////////////////////error/////////////////////////////
    ...
    public void refreshTable() {
        ...
        observableList.addAll(cr.getCreditsByUserId(Model.getInstance().getUser().getId()));
    }
    ...
    public void initialize() {
        ...
        observableList = FXCollections.observableList(cr.getCreditsByUserId(Model.getInstance().getUser().getId()));
    ...
    public Double calculateAndDisplaySumTotalCrWT() {
        double sum = 0.0;


        for (Credit item : tableau.getItems()) {
            sum += (item.getMontant()* item.getInteretMax())/100;
        }
        return sum;
    ...
    void CreditScene(ActionEvent event) {
        try {
            Parent root = FXMLLoader.load(getClass().getResource("com/example/finfolio/User/ajouterCredit.fxml"));
...
// Set the MessageService for OffreController
        ...
        uc.user_parent.setCenter(Model.getInstance().getViewFactory().getoffreView());*/
